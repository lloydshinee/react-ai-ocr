...
function App() {
  ...
  const handleExtract = async () => {
    ...
    // Initialize per-file loading state (so UI shows spinner per card)
    setResults(files.map(() => ({ loading: true })));

    // Process each file independently; update UI as each finishes.
    files.forEach(async (file, i) => {
      try {
        const result = await extractData(file); // ai.js should accept single file
        setResults((prev) => {
        ...
        setResults((prev) => {
          const updated = Array.from(prev);
          updated[i] = { error: err.message || "Unknown error" };
          return updated;
        });
      }
    });
    ...
    <div className="min-h-screen bg-gradient from-slate-50 to-slate-100 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-slate-900 mb-2">Time Card Extractor</h1>
          <p className="text-slate-600">Upload your time card images and extract data automatically</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 mb-8">
          <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition-all group">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="text-xs text-slate-500">PNG, JPG or JPEG (Multiple files supported)</p>
            </div>
        ...
        {previews.length > 0 ? (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-slate-900">Results / Preview</h2>
              ...
              <div key={idx} className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="grid md:grid-cols-2 gap-0">
                  ...
                  <div className="relative bg-slate-50 p-8 flex items-center justify-center min-h-[300px]">
                    <img src={preview.url} alt={preview.name} className="max-w-full max-h-[400px] object-contain rounded-xl shadow-md" />
                    ...
                    {results[idx]?.data && !results[idx]?.loading && (
                      <div className="absolute top-4 left-4 inline-flex items-center gap-2 bg-white/90 px-3 py-1 rounded-full border border-slate-200">
                        ...
                        <span className="text-xs text-slate-700 font-medium">Done</span>
                      </div>
                    )}
                  ...
                  <div className="p-8 flex flex-col">
                    <div className="mb-4">
                      <h3 className="text-lg font-semibold text-slate-900 mb-1">{preview.name}</h3>
                    ...
                    {results[idx]?.error && (
                      <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl w-full">
                        ...
                        <p className="text-sm text-red-800">{results[idx].error}</p>
                      </div>
                    )}
                    ...
                    {results[idx]?.data ? (
                      <div className="flex-1">
                          ...
                          {["ui", "json"].map((mode) => (
                            <button
                              ...
                              className={`px-4 py-2 text-sm font-medium rounded-t-md transition-colors ${
                                viewModes[idx] === mode || (!viewModes[idx] && mode === "ui")
                        ...
                        {viewModes[idx] === "json" ? (
                          ...
                          <div className="bg-slate-50 rounded-2xl p-6 space-y-6">
                            ...
                            {results[idx].data.employee_information && Object.keys(results[idx].data.employee_information).length > 0 ? (
                              <div>
                                <h4 className="text-lg font-semibold text-blue-700 mb-3">Employee Information</h4>
                                <div className="grid sm:grid-cols-2 gap-4 bg-white p-4 rounded-xl border border-slate-200">
                                  {Object.entries(results[idx].data.employee_information).map(([key, value]) => (
                                    <div key={key}>
                                      <p className="text-xs uppercase text-slate-500 font-medium mb-1">{key.replace(/_/g, " ")}</p>
                              ...
                              <p className="text-slate-500 italic text-sm">No employee information found.</p>
                            )}

                            {/* Total Hours (new) */}
                            {results[idx].data.total_hours !== undefined ? (
                              <div>
                                <h4 className="text-lg font-semibold text-blue-700 mb-3">Total Hours</h4>
                                <div className="bg-white p-4 rounded-xl border border-slate-200">
                                  <p className="text-xl font-bold text-slate-900">{results[idx].data.total_hours || "—"}</p>
                                </div>
                              </div>
                            ) : null}

                            {/* Attendance Records */}
                            {Array.isArray(results[idx].data.attendance_records) && results[idx].data.attendance_records.length > 0 ? (
                              <div>
                                <h4 className="text-lg font-semibold text-blue-700 mb-3">Attendance Records</h4>
                                <div className="overflow-x-auto rounded-xl border border-slate-200">
                                  <table className="min-w-full bg-white border-collapse">
                                    ...
                                    <tbody>
                                      {results[idx].data.attendance_records.map((record, i) => (
                                        <tr key={i} className="text-sm hover:bg-slate-50 transition-colors">
                                          {[
                                            ...
                                            <td key={field} className="px-4 py-2 border-b text-slate-700">
                              ...
                              <p className="text-slate-500 italic text-sm">No attendance records found.</p>
                            )}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 flex items-center justify-center">
                        <p className="text-slate-400 text-sm">Click "Extract Data" to analyze this image</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-600">Upload time card images to get started</p>
          </div>
        )}
      </div>
    </div>
